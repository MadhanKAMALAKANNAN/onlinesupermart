...
[assembly: FunctionsStartup(typeof(ShardingAZF.Startup))]
...
namespace ShardingAZF
{
    ...
    //}
        ...
        public override void Configure(IFunctionsHostBuilder builder)
            {
             var loggerFactory = LoggerFactory.Create(loggingBuilder => loggingBuilder.SetMinimumLevel(LogLevel.Trace).AddConsole());


            ILogger logger
                 = loggerFactory.CreateLogger("Trace");
            List<string> loggerList = new List<string>();


            ShardingTimer shardingTimer = new ShardingTimer(loggerList);
            //shardingTimer.ShardingTimerFunction(logger);
