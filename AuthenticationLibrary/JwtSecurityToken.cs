...
namespace AuthenticationLibrary
{
    ...
    public class JwtSecurityToken : SecurityToken
    {
        ...
        public JwtSecurityToken(string jwtEncodedString)
        {
            ...
            if (tokenParts.Length ==  JwtConstants.JwsSegmentCount)
            {
                if (!JwtTokenUtilities.RegexJws.IsMatch(jwtEncodedString))
                    throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12739", jwtEncodedString)));
            }
            else if (tokenParts.Length == JwtConstants.JweSegmentCount)
            {
                if (!JwtTokenUtilities.RegexJwe.IsMatch(jwtEncodedString))
                    throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12740", jwtEncodedString)));
            }
            else
                throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12741", jwtEncodedString)));
        ...
        public JwtSecurityToken(string issuer = null, string audience = null, IEnumerable<Claim> claims = null, DateTime? notBefore = null, DateTime? expires = null, SigningCredentials signingCredentials = null)
        {
            if (expires.HasValue && notBefore.HasValue)
            {
                if (notBefore >= expires)
                    throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12401","",""// LogHelper.MarkAsNonPII(expires.Value), LogHelper.MarkAsNonPII(notBefore.Value)
                        )));
            }
        ...
        internal void Decode(string[] tokenParts, string rawData)
        {
            ...
            {
                throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12729", tokenParts[0], rawData), ex));
            }
        ...
        private void DecodeJws(string[] tokenParts)
        {
            ...
            if (Header.Cty != null)
                LogHelper.LogVerbose(LogHelper.FormatInvariant("IDX12738", Header.Cty));
            ...
            {
                throw LogHelper.LogExceptionMessage(new ArgumentException(LogHelper.FormatInvariant("IDX12723", tokenParts[1], RawData), ex));
            }
